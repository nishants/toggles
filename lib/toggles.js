(function(){
  ...
  app.service("toggles", ["$location", "$http", function($location, $http){
    ...
    var setConfig = function (response) {
      var config = response,
    ...
    return {
      init : function(promise){
        getConfig = promise.then(setConfig);
      },
      getConfig : function(){
        if(!getConfig){
          getConfig = $http.get("config/features.json").then(function(response){return response.data}).then(setConfig);
        }
        return getConfig;
      }
    }
